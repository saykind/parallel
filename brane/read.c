...
int main(int argc, char *argv[]) {
    ...
    *(c+1) = (int **)malloc(L*sizeof(int*));
        ...
        for (i = 0; i < L; i++)
            for (j = 0; j < L; j++) {
                ...
                if (c[1][i][j]) {
                    g[i][j]  /= c[1][i][j];
                    g2[i][j] /= c[1][i][j];
                    g2[i][j] = 2*sqrt(fabs((g2[i][j]-g[i][j]*g[i][j])/c[1][i][j]));
                    if (verbose)
    ...
    for (i = 1; i < N; i++) {
            fprintf(file,"%lf\t%lf\t%lf\n", i*a, 1./g[0][i], g2[0][i]/g[0][i]);
            fprintf(file,"%lf\t%lf\t%lf\n", i*a, 1./g[i][0], g2[i][0]/g[i][0]);
            fprintf(file,"%lf\t%lf\t%lf\n", sqrt(2)*i*a, 1./g[i][i], g2[i][i]/g[i][i]);
        }
    fclose(file);
	...
	fprintf(gpp, "plot x**4, x**4*(1+(%f/x)**2)**(.4), x**4*(%f/x)**.8, '-' w errorbars pt 7 ps .5\n", p8, p8);
	for (i = 1; i < N+1; i++)
